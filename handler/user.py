...
class SettingAvatarHandler(BaseHandler):
    ...
    def post(self, template_variables = {}):
        ...
        if(not "avatar" in self.request.files):
            ...
            template_variables["errors"]["invalid_avatar"] = [u"请先选择要上传的靓照"]
            self.get(template_variables)
            return
        ...
        template_variables["success_message"] = [u"同学靓照更新成功"]
        # update `updated`
...
class SettingPasswordHandler(BaseHandler):
    ...
    def post(self, template_variables = {}):
        ...
        if(not user_info["password"] == secure_password):
            template_variables["errors"] = {}
            template_variables["errors"]["error_password"] = [u"sorry,当前密码输入有误"]
            self.get(template_variables)
            return
...
class ForgotPasswordHandler(BaseHandler):
    ...
    def post(self, template_variables = {}):
        ...
        mail_title = u"径成校区（vvovo.com）找回密码"
        template_variables = {"email": form.email.data, "new_password": new_password};
        template_variables["success_message"] = [u"新密码已发送至您的注册邮箱,请注意查收。"]
...
class RegisterHandler(BaseHandler):
    ...
    def post(self, template_variables = {}):
        ...
        if(duplicated_email or duplicated_username):
            ...
            if(duplicated_email):
                template_variables["errors"]["duplicated_email"] = [u"sorry,所填邮箱已经被注册过"]

            if(duplicated_username):
                template_variables["errors"]["duplicated_username"] = [u"sorry,所填用户名已经被注册过"]
        ...
        if(form.username.data in self.settings.get("reserved")):
            template_variables["errors"] = {}
            template_variables["errors"]["reserved_username"] = [u"sorry,用户名被保留不可用"]
            self.get(template_variables)
            return
        ...
        # validate college name
         ...
         #   template_variables["errors"] = {}
          #  template_variables["errors"]["duplicated_collegename"] = [u"学校名称不正确（进入所有学校页面 vvovo.com/colleges 核对你想输入的学校名称）"]
           # self.get(template_variables)
           # return
